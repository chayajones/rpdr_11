...
wrangled <- all_rankings_next %>%
  ...
  group_by(contestant_id) %>% 
    ...
    #mutate(past_lipsync = cumsum(episode_placement == "BTM2") - (episode_placement == "BTM2")) %>%
    #mutate(past_top = cumsum(episode_placement == "HIGH") - (episode_placement == "HIGH")) %>%
    #mutate(past_bottom = cumsum(episode_placement == "BTM2" | episode_placement == "LOW") - (episode_placement == "BTM2" | episode_placement == "LOW")) %>%
  ungroup() %>%
  ...
  select(season_number, episode_number, t, contestant_id, contestant_name, # identifiers
         ethnicity, asian, black, latino, white, z.age, z.past_wins, z.past_lipsync, z.past_top, z.past_bottom, plus_size, drag_house_ind, drag_fam_competed, puerto_rican, new_york_city, # x variables
...
model {
  ...
  vector[N] eta = beta_asian * asian + beta_latino * latino + beta_puerto_rican * puerto_rican + beta_drag_house_ind * drag_house_ind + beta_past_top * past_top + beta_past_bottom * past_bottom + alpha_contestant[ii];
...
```
```{r stan_plot}
print(fit, pars = c('beta_asian', 'beta_latino', 'beta_drag_house_ind', 'beta_puerto_rican', 'beta_past_top', 'beta_past_bottom', 'sigma'))
stan_plot(fit, pars = c('beta_asian', 'beta_latino', 'beta_drag_house_ind', 'beta_puerto_rican', 'beta_past_top', 'beta_past_bottom', 'sigma')) + geom_vline(xintercept=0, size = 2)
```
...
for (t_current in (L+1):(next_t - 1)) {
  ...
  for (n in 1:nrow(newdata)) {
    ...
    eta_s[,n] <- s$beta_asian * newdata$asian[n] + s$beta_latino * newdata$latino[n] + s$beta_drag_house_ind * newdata$drag_house_ind[n] + s$beta_puerto_rican * newdata$puerto_rican[n] + s$beta_past_top * newdata$z.past_top[n] + s$beta_past_bottom * newdata$z.past_bottom[n] + alpha_contestant_s
  }
