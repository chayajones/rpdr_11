...
all_episodes_next <- add_row(all_episodes, season_number = 11, episode_number = 3,
...
all_rankings_next <- add_row(all_rankings, season_number = 11, episode_number = 3, 
...
wrangled <- all_rankings_next %>%
  ...
  mutate(placement = case_when(is.element(episode_placement,c('WIN','Winner')) ~ 1,
                      ...
                      TRUE ~ 0)) %>%
  mutate(lipsync = case_when(is.element(episode_placement, c('BTM2')) ~ 1,
                      TRUE ~ 0)) %>%
  mutate(top = case_when(is.element(episode_placement, c('HIGH', 'WIN', 'Winner')) ~ 1,
                      TRUE ~ 0)) %>%
  mutate(bottom = case_when(is.element(episode_placement, c('BTM2', 'LOW')) ~ 1,
                      TRUE ~ 0)) %>%     
...
model {
  ...
  // beta_asian * asian + beta_latino * latino + 
  // vector[N] eta = beta_asian * asian + beta_latino * latino + beta_drag_house_ind * drag_house_ind + beta_past_high * past_high + beta_past_low * past_low + beta_past_lipsync * past_lipsync + beta_past_wins * past_wins + alpha_contestant[ii];
  // vector[N] eta = beta_past_high * past_high + beta_past_low * past_low + beta_past_lipsync * past_lipsync + beta_past_safe * past_safe + beta_past_wins * past_wins + alpha_contestant[ii];
  vector[N] eta = beta_asian * asian + beta_latino * latino + beta_drag_house_ind * drag_house_ind + beta_past_top * past_top + beta_past_bottom * past_bottom + alpha_contestant[ii];
...
print(fit, pars = c('beta_asian', 'beta_latino', 'beta_drag_house_ind', 'beta_past_top', 'beta_past_bottom', 'sigma'))
stan_plot(fit, pars = c('beta_asian', 'beta_latino', 'beta_drag_house_ind', 'beta_past_top', 'beta_past_bottom', 'sigma')) + geom_vline(xintercept=0, size = 2)
```
...
for (t_current in (L+1):(next_t - 1)) {
  ...
  for (n in 1:nrow(newdata)) {
    ...
    eta_s[,n] <- s$beta_asian * newdata$asian + s$beta_latino * newdata$latino + s$beta_drag_house_ind * newdata$drag_house_ind + s$beta_past_top * newdata$z.past_top[n] + s$beta_past_bottom * newdata$z.past_bottom[n] + alpha_contestant_s
  }
