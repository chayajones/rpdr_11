...
model {
  ...
  vector[N] eta = beta_asian * asian + beta_latino * latino + beta_puerto_rican * puerto_rican + beta_drag_house_ind * drag_house_ind + beta_past_high * past_high + beta_past_low * past_low + beta_past_lipsync * past_lipsync + beta_past_wins * past_wins + alpha_contestant[ii];
...
print(fit, pars = c('beta_asian', 'beta_latino', 'beta_puerto_rican', 'beta_drag_house_ind', 'beta_past_wins', 'beta_past_lipsync', 'beta_past_high', 'beta_past_low', 'sigma'))
stan_plot(fit, pars = c('beta_asian', 'beta_latino', 'beta_puerto_rican', 'beta_drag_house_ind', 'beta_past_wins', 'beta_past_lipsync', 'beta_past_high', 'beta_past_low', 'sigma')) + geom_vline(xintercept=0, size = 2)
```
...
for (t_current in (L+1):(next_t - 1)) {
  ...
  for (n in 1:nrow(newdata)) {
    ...
    eta_s[,n] <- s$beta_asian * newdata$asian + s$beta_latino * newdata$latino + s$beta_puerto_rican * newdata$puerto_rican + s$beta_drag_house_ind * newdata$drag_house_ind + s$beta_past_high * newdata$z.past_high[n] + s$beta_past_low * newdata$z.past_low[n] + s$beta_past_wins * newdata$z.past_wins[n] + s$beta_past_lipsync * newdata$z.past_lipsync[n] + alpha_contestant_s
  }
